...
const NavItem = ({ item, level, drawerToggle }) => {
  ...
  let extractedPath = '';
  if (location.pathname !== '/dashboard/default') {
    extractedPath = location.pathname.split('/').slice(0, 2).join('/');
  }
  ...
  return (
    <ListItemButton
      ...
      selected={location.pathname === item.url || extractedPath === item.url}
      component={Link}
      onClick={isMobile ? drawerToggle : handleClick}
      to={item.url}
      target={itemTarget}
      {...listItemProps}
    >
